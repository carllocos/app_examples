...
>[!IMPORTANT]\
> The following assumes that [wasmito](https://github.com/carllocos/wasmito) was installed
 

To deploy a Wasm module into a MCU, you can make use of the `arduino-cli` command installed upon installation of Wasmito,
or use the CLI provided by Wasmito, which uses behind the scenes the `arduino-cli` command.


In the following, we show how to deploy an application implemented in AssemblyScript.
For other languages, similar steps can be applied.
Concretely, we how to compile and deploy a Blinking LED project into an M5StickC.
